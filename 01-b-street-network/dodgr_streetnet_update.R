# https://rdrr.io/cran/dodgr/src/R/dodgr-streetnet.R
...
#' This must be in ISO8601 format ("YYYY-MM-DDThh:mm:ssZ"), where both the "T" 
#' and "Z" characters must be present. (passed to osmdata::opq)
...
#' 
library(dplyr)
dodgr_streetnet_update <- function (bbox,
                             datetime = NULL, # added this feature to allow pull of street network at specific datetime using opq functionality
                             pts = NULL,
                             expand = 0.05,
                             quiet = TRUE) {
    ...
    net <- osmdata::opq (bb$bbox,datetime=datetime) %>%
...
#' @export
dodgr_streetnet_sc_update <- function (bbox,
                                datetime = NULL, # added this feature to allow pull of street network at specific datetime using opq functionality
                                pts = NULL,
                                expand = 0.05,
                                quiet = TRUE) {
    ...
    osmdata::opq (bb$bbox, datetime = datetime) %>%
